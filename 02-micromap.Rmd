...
presence of islands or in case a land mass is split into more than one area, e.g., 
as in the state of Michigan in the US.
...
adults with four or more years of college), the correlation is relatively weak.
...
used in the following linked micromap plot\index{Linked micromap plot}, rather than having
...
Numerous R packages, such as **rgdal**\index{R Packages!rgdal} and
**shapefiles**\index{R Packages!shapefiles}, support the use of shapefiles\index{Shapefiles} in R.
...
_edPov_\index{Datasets!edPov} is a data frame that contains
education and poverty level data for the 50 states (and Washington, D.C.).
This data frame has 51 rows, one for each of the 50 states (and for Washington, D.C.).
We have to identify one variable from this data frame that can be used for linking the
two data sets. Here, this is the `StateAb` variable.
