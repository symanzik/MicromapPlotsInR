...
In Section \@ref(Ch2-Steps), we will introduce the
four main steps that are required to create a 
...
"
  ...
  charToRaw() -> micromapFlowchart

if (knitr::is_latex_output()) {
  rsvg::rsvg_pdf(svg = micromapFlowchart, file = "img/Ch2-flowchart.pdf")

  knitr::include_graphics("img/Ch2-flowchart.pdf")
} else {
  rsvg::rsvg_png(svg = micromapFlowchart, file = "img/Ch2-flowchart.png")

  knitr::include_graphics("img/Ch2-flowchart.png")
}
```
...
stored in a simple features\index{Simple features} (sf) format [@Pebesma2018;@OGC2022] instead of a
SpatialPolygonsDataFrame.\index{SpatialPolygonsDataFrame}
From a user's perspective, we think that the approach that links 
a data frame with the statistical data to another data frame with the underlying 
spatial boundary data is the easier approach for most users.
First, this approach is most intuitive if the statistical data is stored
in an external Excel file that is read into a data frame in R before 
any linked micromap plot\index{Linked micromap plot} is created.
Second, this approach does not require any familiarity 
with spatial objects in R and spatial data in general.
For more advanced users, we recommend experimenting with the approach
that is based on the simple features\index{Simple features} format,
in particular if the external shapefile\index{Shapefile} contains both,
the statistical data and the boundary data.
While most of the linked micromap plots\index{Linked micromap plot} in this book
that are created via the **micromap**\index{R Packages!micromap} R package
make use of the more traditional approach that links the two data frames,
Example 2 (in Section \@ref(Ch2-Example2)) demonstrates how both approaches
can be used for the same underlying data sets. 
One example in Section \@ref(Ch11-Spatial-Surveys) directly creates
a linked micromap plot\index{Linked micromap plot} based on the
simple features\index{Simple features} option.
...
function as outlined in Section \@ref(Ch2-Example1),
...
Additional details and examples for both options can also be found in Section \@ref(Ch2-Example2)
and in the vignette [@PBWMOK2024Vignette] of the **micromap**\index{R Packages!micromap} R package.
