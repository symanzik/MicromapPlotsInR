...
the statistical data represented as dot plots\index{Dot plot} (`dot`) in the third and fourth columns,
...
the dot plots\index{Dot plot} in the two statistical graphics columns,
in the linked micromap plot\index{Linked micromap plot} shown in
...
For use in linked micromap plot\index{Linked micromap plot}
...
that has to be split into the geographic information and the statistical data components.
Numerous R packages, such as **rgdal**\index{R Packages!rgdal} [@BKR2022] and
**shapefiles**\index{R Packages!shapefiles} [@Stabler2022], support the use of shapefiles\index{Shapefiles} in R.
It is worthwhile to mention here that geography and attributes that are
...
**rgdal**\index{R Packages!rgdal} R package [@BKR2022] as discussed in the previous section. It contains
...
As before, we want to verify that there is indeed at least one matching ID 
...
the first variable name (`Random_Wat`) must come from the statistical data (`wv_data`)
and the second variable name (`ID`) must come from the `wv_polys_table` data frame that
contains the spatial boundary information.
Changing the order of these two variable names typically results in an error.

As discussed in Section \@ref(Ch2-Creating),
the `panel.types` and `panel.data` are closely related. 
Even though we want to ultimately display boxplots and dot plots in the
statistical graphics columns of the linked micromap plot\index{Linked micromap plot},
it often is prudent to start with simple dot plots\index{Dot plot}.
...
for the third and fourth column. No further data has to be used to the
...
We abstain from interpreting this figure at this stage, but we will provide some
helpful interpretation once we have created the refined version of this figure
in the next step.
...
First, we get rid of the eleventh perceptual group (with just one subregion) and introduce
a median row via `median.row = TRUE` instead. A median row is often a good solution if the 
number of subregions is odd such as for the 50 states (and Washington, D.C.) here.
Here, Wyoming is the state shown in the median row. It has the 
26$^{th}$ highest (or lowest) value for the sorting variable, i.e., `pov`.
It does not appear in a map by itself, but rather is added to the perceptual
groups above and below the median row in a neutral color, 
