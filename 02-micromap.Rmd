...
the first variable name (`Random_Wat`) must come from the statistical data data frame (`wv_data`)
...
and dot plots with confidence bounds\index{Dot plot with confidence bounds} in the
statistical graphics columns of the linked micromap plot,\index{Linked micromap plot}
...
A problem that did not exist in the first example in Section \@ref(Ch2-Refining) is the
...
This is done via `box_summary` and `dot_cl` for the `panel.types`.
A boxplot\index{Boxplot} requires five components 
(minimum, first quartile, median, third quartile, and maximum)
and a dot plots with confidence bounds\index{Dot plot with confidence bounds}
...
for the boxplot\index{Boxplot} and
`list("Cond_med", "Cond_LCB95", "Cond_UCB95")`
for the dot plots with confidence bounds.\index{Dot plot with confidence bounds}
...
```{r Ch2-refining1WV, fig.cap = 'First refined linked micromap plot\\index{Linked micromap plot}, based on the `wv_data` data frame. Main changes are the introduction of abbreviated subregion names and initial boxplots\\index{Boxplot} and dot plots with confidence bounds\\index{Dot plot with confidence bounds} in the two statistical graphics columns.', fig.width = 7, fig.height = 7}
mmplot(
  stat.data = wv_data,
  map.data = wv_polys_table,
  map.link = c("Random_Wat", "ID"),
