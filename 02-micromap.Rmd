...
will be introduced in this chapter. The reader will be introduced to the
...
for the 50 U.S. states (and Washington, D.C.). What we want to discuss next is the choice of colors
...
largest (at the bottom) median observed conductivity in a certain subbasin. 
...
This includes a reverse sorting (`rev.ord = TRUE`) that places the largest observed median conductivity on top, 
...
The value `129` microSiemens per cm ($\mu$S/cm) represents the estimated median conductivity 
...
mmplot(
  ...
  panel.att = list(
    ...
    list(
      ...
      xaxis.title = "Conductivity [1,000 \u03BCS/cm]", 
    ...
    list(
      ...
      xaxis.labels.size = 1.25,
      xaxis.title = "Conductivity [\u03BCS/cm]",
      add.line = 129,
      add.line.col = "black", 
      add.line.typ = "dashed",
      xaxis.title.size = 1.25, 
...
The rows in the plot are sorted from highest (on top) to lowest (at the bottom) 
median observed conductivity in a certain subbasin.

Each consecutive map shows which subregions have been colored previously in the map(s) above. 
There is a weak spatial pattern with the highest median observed conductivities in the northern and 
western subbasins of West Virginia. There is a notable spatial cluster in the 
central-eastern subbasins of West Virginia with the five lowest median observed conductivities
...
estimated conductivity, the wider the confidence bound in general.  The value 129 $\mu$S/cm, 
...
Here, some extreme outliers dominate, e.g., observed conductivities of about 5,000 $\mu$S/cm
