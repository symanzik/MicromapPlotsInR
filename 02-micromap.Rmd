...
UPDATE _USstates_\index{Datasets!USstates} and _edPov_\index{Datasets!edPov} datasets
from the **micromap**\index{R Packages!micromap} R package. ENDUPDATE

We follow the four steps outlined in Section \@ref(Ch2-Steps) again.
...
UPDATE ALL TEXT AND FURTHER REFINE THE LINKED MICROMAP PLOT.
...
```{r Ch2-WV, fig.cap = 'Map representation of the _USstates_\\index{Datasets!USstates} spatial boundary data set for the United States that frequently is used as the basis for linked micromap plots\\index{Linked micromap plot} that are created with the **micromap**\\index{R Packages!micromap} R package.', fig.width = 7, fig.height = 4}
library(micromap)
...
head(wv_watershed@data)

wv_data <- wv_watershed@data # Pull out statistical data from spatial polygon dataframe
head(wv_data)
names(wv_data)
```
...
the lower 95\% confidence bound, and the upper 95\% confidence bound of a variable, respectively.
These will be used for the construction of dot plots with confidence bounds.
...
third quartile, and maximum of a variable, respectively.
These will be used for the construction of boxplots.
