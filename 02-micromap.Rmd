...
Eventually, Marcus W. Beck, then also with the USEPA in Corvallis, Oregon,
...
has been released to CRAN in June 2013 [@CP2013CRAN], i.e., 
...
Ultimately, one has to make one's own decision which of these two R packages
...
This chapter comes to a conclusion with a summary and suggestions for further reading
...
All of these R packages should
...
have to be used for linking the two datasets. For the _USstates_\index{Datasets!USstates} dataset,
...
only data for the 50 U.S. states but not for Washington, D.C., mismatching identifiers also can be
...
that there are 51 subregions overall: The 50 U.S. states and Washington, D.C.
...
What we want to discuss next is the choice of colors
...
stored in a simple features\index{Simple features} format [@OGC2022] instead of a
SpatialPolygonsDataFrame\index{SpatialPolygonsDataFrame}
...
(ref:Ch2-WV-cap) Map representation of the _WV_Watershed_\index{Datasets!WV\_Watershed} spatial boundary dataset for the 25 watersheds in West Virginia that is used as the basis for the linked micromap plots\index{Linked micromap plot} in this section.
...
```{r Ch2-creatingdraftWVdot, fig.cap = 'Draft linked micromap plot\\index{Linked micromap plot}, based on the previously created `wv_data` and `wv_polys_table` data frames for the West Virginia watershed dataset.', fig.width = 7, fig.height = 7}
mmplot(
  stat.data = wv_data,
...
Similar to Figure \@ref(fig:Ch2-creatingdraftWVdot), 
the same draft linked micromap plot,\index{Linked micromap plot}
shown in Figure \@ref(fig:Ch2-creatingdraftWVdotsf),
can be created directly from the shapefile that also contains the data. 
It is not necessary to call the `create_map_table()` function first.
Also, the `stat.data` and `map.link` arguments are not needed as the `map.data` object
contains all relevant geographic and statistical data.
This draft linked micromap plot\index{Linked micromap plot}
could be refined in a similar way as outlined in Section \@ref(Ch2-RefiningWV).
We leave it to the reader to modify the subregion names in the sf object before creating
the final refined linked micromap plot\index{Linked micromap plot} 
that resembles the one in \@ref(fig:Ch2-refining2WV).


```{r Ch2-creatingdraftWVdotsf, fig.cap = 'Draft linked micromap plot\\index{Linked micromap plot}, based on the West Virginia watershed dataset. In contrast to the previous figure, this figure has been created directly from the external shapefiles without using the `create_map_table()` function first.', fig.width = 7, fig.height = 7}
...
although the **micromapST**\index{R Packages!micromapST} R package [@PeCa2022CRAN201]
...
Chapter \@ref(Ch3) introduces the **micromapST**\index{R Packages!micromapST} R package [@CP2015CRAN].
...
Each of the two R packages has its own strengths and weaknesses.
