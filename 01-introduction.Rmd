...
that are used to show spatial patterns. These maps are also used to highlight 
...
After introducing the three main types of micromaps,
...
of the three major classes for micromaps. 
...
carbon dioxide (CO~2~) emissions in
...
based on several other visualization developments of the 1980s and 1990s. In particular,
...
traditionally used choropleth maps where the map regions are filled by a
continuous or discrete color scheme.
First, small map regions (such as Washington, D.C., in a map of the 
United States) may be hard to see in a choropleth map.
In linked micromap plots,\index{Linked micromap plot} small map regions
are often enlarged, resulting in some kind of map caricatures 
where such a region is enlarged and often also pulled to the outside of the main map.
...
discrete colors for coloring the map area in a choropleth map, 
...
and the relative proximities of the values translated into a certain color class 
that represents an interval of values is lost. This problem is solved by
using row-label statistical displays such as dotplots\index{Dotplot}
that link to the map in linked micromap plots,\index{Linked micromap plot}
rather than showing the statistical data directly on the map.

Third, it is difficult to show more than one statistical variables directly
...
that make up the summary statistics visually displayed in a boxplot.\index{Boxplot}
This problem is resolved in linked micromap plots\index{Linked micromap plot}
by linking the statistical graphic display to certain spatial areas,
rather than displaying the statistical information directly on the map.
...
@SC2008 and @SCMW2017, respectively. Even more, 
for readers who want to learn the full background behind all three main types
of micromaps, we strongly recommend to read the excellent book by @CP2010.

We will use two mock-up linked micromap plots\index{Linked micromap plot},
shown in Figures \@ref(fig:Ch1-NJ-Layout1) and \@ref(fig:Ch1-NJ-Layout2),
to introduce the main nomenclature for linked micromap plots\index{Linked micromap plot}
and how to interpret such plots. These two figures are based
on actual boundaries for New Jersey in the United States, obtained from
@USCensus2013Shapefiles. The statistical data were entirely made up for demonstration purposes.


```{r Ch1-NJ-Layout1, fig.cap = 'Mock-up linked micromap plot\\index{Linked micromap plot}, based on the 21 counties of New Jersey.', fig.width = 7, fig.height = 8, echo = FALSE}
library(micromap)
library(raster)
...
mmplot(
  ...
  panel.types = c("map", "dot_legend", "labels", "dot", "dot", "dot"),
  panel.data = list(NA, NA, "County", "StatVar1", "StatVar2", "StatVar3"),
  ord.by = "StatVar1",
  rev.ord = TRUE,
...
```{r Ch1-NJ-Layout2, fig.cap = 'Mock-up linked micromap plot\\index{Linked micromap plot}, based on the 21 counties of New Jersey. Main changes are the use of two-ended cumulative maps, the reduced number of perceptual groups, the introduction of a median row, and using `StatVar2` as the sorting variable.', fig.width = 7, fig.height = 8, echo = FALSE}
# create linked micromap plot
mmplot(
  stat.data = nj_data,
  map.data = nj_polys_table,
...
of observed or estimated statistical data. Sometimes, a global summary statistic is
also shown in these columns. The three remaining columns are used for the maps,
a plot symbol, and the subregion names, abbreviations, or 
some other meaningful identifiers. The subregions are sorted according to a
sorting variable that may or may not be one of the variables that are displayed
in the statistical graphics columns. There exists no fixed order how the columns have to be arranged.
...
The rows in linked micromap plots\index{Linked micromap plot}, representing the subregions, are arranged in
...
Table XXX provides suggestions for the number of subregions in each perceptual group,\index{Perceptual group}
...
is ideal from a cognitive perspective.
Within each perceptual group,\index{Perceptual group} the subregions shown in the map,
the plot symbol next to the subregion names, and the graphics in the statistical graphics columns are linked
by color. This allows to quickly link information from the statistical graphics columns
to the underlying subregions in the maps and to the subregion names via the plot symbol. Within each 
perceptual group,\index{Perceptual group} a particular color represents a single
subregion in the map column, the same color is used for the plot symbol next
to the subregion name, and for the graphs in the statistical graphics columns.
...
Figures \@ref(fig:Ch1-NJ-Layout1) and \@ref(fig:Ch1-NJ-Layout2) each show a six-column
linked micromap plot\index{Linked micromap plot} for the 21 counties of New Jersey.
The first column shows the maps, the second column shows the plot symbol,
and the third column shows the subregion identifiers, here the county names in New Jersey.
Columns four, five, and six are the statistical graphics columns 
...
from highest (on top) to lowest (at the bottom) according to `StatVar1`.
...
from highest (on top) to lowest (at the bottom) according to `StatVar2`.
This layout makes use of a median row.
...
and from the bottom perceptual group to the median row only
by sequentially
...
high values or `StatVar1` are associated with high values of `StatVar2` and
low values or `StatVar1` are associated with low values of `StatVar2`.
...
high values or `StatVar1` (and `StatVar2`) are associated with low values of `StatVar3` and
low values or `StatVar1` (and `StatVar2`) are associated with high values of `StatVar3`.
...
```{r Ch1-NJ-Scatterplot, fig.cap = 'Mock-up scatterplot of the three statistical variables shown in the two previous linked micromap plots\\index{Linked micromap plot}, based on the 21 counties of New Jersey.', fig.width = 5, fig.height = 5, echo = FALSE}
graphics::pairs(x = nj_data[, 2:4])
```
