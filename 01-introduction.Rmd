...
NOTE:
New Jersey shapefiles obtained from
https://www2.census.gov/geo/tiger/PREVGENZ/co/co00shp/
on 10/17/2022

NJ car crash data
https://www.sciencedirect.com/science/article/pii/S2665963821000762



```{r Ch1-NJ-Layout1, fig.cap = 'New Jersey', fig.width = 3, fig.height = 5}
library(raster)
library(dplyr)
library(micromap)
...
proj4string(nj_shapefile) <- "+proj=longlat +ellps=WGS84"
...
class(countypop)
head(countypop)
nj_countypop <- countypop[countypop$abbr == "NJ", ]
nj_countypov <- countypov[countypop$abbr == "NJ", ]

nj_countypopfips <- cbind(nj_countypop, substr(countypop[countypop$abbr == "NJ", ]$fips, 3, 5))
names(nj_countypopfips)[5] <- "COUNTY"

nj_countypovfips <- cbind(nj_countypov, substr(countypov[countypop$abbr == "NJ", ]$fips, 3, 5))
names(nj_countypovfips)[5] <- "COUNTY"

nj_data <- nj_shapefile@data

nj_polys_table <- create_map_table(
  tmp.map = nj_shapefile,
  IDcolumn = "COUNTY"
)

nj_data_new <- inner_join(nj_data, nj_countypopfips) %>%
  inner_join(nj_countypovfips)
...
# Get 2019 accidents
ac19 <- get_njtr1(year = 2019, type = "Accidents", geo = TRUE) %>%
...
# Get 2020 accidents
ac20 <- get_njtr1(year = 2020, type = "Accidents", geo = TRUE) %>%
  filter(is.na(latitude) == FALSE) %>%
  mutate(longitude = longitude*(-1)) %>%
  st_as_sf(coords = c("longitude","latitude"),crs = 4326, remove = FALSE) %>%
  st_transform(3424)

ac19_county <- ac19 %>% st_drop_geometry() %>% group_by(county_name) %>% tally() %>% mutate(NAME = str_to_title(county_name), ac19 = n) %>% select(NAME = NAME, ac19 = ac19)

ac20_county <- ac20 %>% st_drop_geometry() %>% group_by(county_name) %>% tally() %>% mutate(NAME = str_to_title(county_name), ac20 = n) %>% select(NAME = NAME, ac20 = ac20)

ac19_county$ac19 - ac20_county$ac20 


nj_data_new2 <- inner_join(nj_data_new, ac19_county) %>%
  inner_join(ac20_county) %>%
  mutate(acbypop19 = ac19 / pop_2015,
         acbypop20 = ac20 / pop_2015,
         acdiff = ac19 - ac20,
         acdiffbypop = acdiff / pop_2015)

nj_data_new2

# ac19_county <- ac19_ts %>% group_by(county_name) %>% tally()

mmplot(
  stat.data = nj_data_new2,
  map.data = nj_polys_table,
  map.link = c("COUNTY", "ID"),
  panel.types = c("dot_legend", "labels", "dot", "dot", "dot", "map"),
  panel.data = list(NA, "NAME", "acdiffbypop", "acbypop19", "acbypop20", NA),
  ord.by = "acdiffbypop", 
