...
the subregion identifiers, and a plot symbol. The subregions are sorted according to a
...
The rows in linked micromap plots\index{Linked micromap plot} are arranged in
perceptual groups. Ideally, each perceptual group should contain the same
number of subregions, but that is not always possible. 
Table XXX provides suggestion for the number of subregions in each perceptual group,
depending on the total number of subregions in the area of interest.
In practice, having close to five subregions in most of the perceptual groups
is ideal from a cognitive perspective.

Figures \@ref(fig:Ch1-NJ-Layout1) and \@ref(fig:Ch1-NJ-Layout2) show six-column
linked micromap plots\index{Linked micromap plot} for New Jersey.
