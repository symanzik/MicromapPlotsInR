...
throughout the book, and introduce the primary software that is available
...
of the three major classes of micromaps. 
...
various branches of the U.S. Environmental Protection Agency (EPA),
the National Cancer Institute (NCI),
the U.S. Census Bureau, and
the U.S. Bureau of Labor Statistics (BLS).
In addition to static linked micromap plots,\index{Linked micromap plot}
some of these agencies (in particular the NCI, the EPA, and the USADA-NASS) developed early
...
@SC2008 also pointed out three main advantages of 
linked micromap plots\index{Linked micromap plot} over
traditionally used choropleth maps where the map regions are filled by colors from a
continuous or discrete color scheme to display numerical information on the map.
...
where such a subregion is enlarged and sometimes also is pulled to the outside of the main map.
...
```{r Ch1-NJ-Scatterplot, fig.cap = 'Mock-up scatterplot of the four statistical variables shown in the two previous linked micromap plots\\index{Linked micromap plot}, based on the 21 counties of New Jersey.', fig.width = 5, fig.height = 5, echo = FALSE}
