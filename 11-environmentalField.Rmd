...
Two final points about the WVDEP linked micromap should be noted. First, we used a simple features object, **sf**\index{R Packages!sf}, that contained both the geospatial data and the statistical of the basins. Consequently, we only had to specify the `map.data` argument, in contrast to the Virginia and Florida examples, where we used both the `map.data` and `stat.data` syntax.  Second, the basins in the linked micromap were smoothed, that is their polygon boundaries generalized, so they would draw faster as mention in \@ref(Ch2).
