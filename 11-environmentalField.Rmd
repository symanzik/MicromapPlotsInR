...
Finally, the R code used to create Figure \@ref(fig:Ch11-micromapVDEQ) is shown below, which uses the _stats_\index{Datasets!stats} and _map.table_\index{Datasets!map.table} datasets.
...
We begin by plotting the Oregon border and lakes together in Figure \@ref(fig:Ch11-micromap-oregon-lakes). Note that both spatial layers are simple features objects and the same coordinate reference system or crs is required to plot them together. This condition is evaluated by `st_crs(OR) == st_crs(OR_lakes)`.  
