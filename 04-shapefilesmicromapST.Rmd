...
BuildBorderGroup(
  ...
  MapHdr = c("", "States"),
  debug = 0 #  = 512 + 128 to generate 4 intermediate PNG map plots
)
...
We will use cancer data from the state of Kentucky, with 120 counties, to illustrate a long linked micromap plot display. Kentucky had the highest invasive cervical cancer rate among women in the U.S. for the period 2014-2018 [ref https://www.kcr.uky.edu/, Kentucky Cancer Registry, accessed 9/27/22]. There is a test to detect this cancer at an early stage and a vaccine to prevent infections that can lead to cancer later. In an effort to reduce these high rates, the state public health department would probably be interested in examining geographic patterns within the state. A linked micromap plot of cervical cancer incidence rates can display these patterns and help the state target their scarce resources to where they are needed most.
...
KY_spdf <- readOGR(KYADD_dir, KY_shp_file, verbose = FALSE)
...
load(paste0(KYADD_dir,"/KY_countiesBG.rda"))   # this loads new BG
PlotVis(areaVisBorders)
...
KY_cervca <- read.csv(paste0(KYADD_dir,"/Invasive-Cancer-Incidence-Rates-by-County-in-Kentucky-Cervix-Uteri-2014-2018.csv"), skip = 2)
head(KY_cervca)
...
temp_name_table <- as.data.frame(read_xlsx(paste0(KYADD_dir,"/KYADD_NameTable-Basic.xlsx")))
...
BuildBorderGroup(
  ...
  BorderGroupName = "KYADDBG", # name of border group to use
...
pdf(paste0(BGDir, "/KYADD.pdf"), width = 8, height = 4)
...
``` {r Ch4b-KY-ADD-LMM, fig.cap = "Invasive cervical cancer incidence rates in Kentucky Area Development Districts, 2014-2018, age-adjusted to 2000 standard million population", fig.width = 7, fig.height = 10}
#  Read in KY cervical cancer incidence rate data (skip 1st 2 header rows)
KYADD_cervca <- read.csv(paste0(KYADD_dir,"/Invasive-Cancer-Incidence-Rates-by-ADD-in-Kentucky-Cervix-Uteri-2014-2018.csv"), skip = 2)
head(KYADD_cervca)
