...
to geographic areas for which the associated spatial data
...
\begin{enumerate}
  ...
  and the `points2circles()` function provides a convenient wrapper to do this.
  \item The base map and polygons representing point locations need to be 
  combined into single object.
  \item Flatten the object created in the previous step into a dataframe 
...
Next we load thad the point location data. 

```{r}
cities <- readRDS("data/Ch6-data/asian_cities.RDS")
```

We can conver the population column to be in units of millions of people, 
...
The next step is to draw polygons (circles) around the points. Here we 
achieve this using the `points2circles` function. This can be supplied
a dataframe with columns for the x and y coordinates, or a simple features object
(from the `sf` package).
...
The `mmplot` requires a dataframe as input, which can be created
using `create_map_table` (see chapters
X, Y, Z). The 
`sf` object first needs to converted to a `Spatial` object (from the `sp` package),
...
Before the objects can be combined, the `asia` object needs to be converted to an `sf` object. 

```{r}
asia_sf <- st_as_sf(asia)
```

Before they can be combined, the two objects need to share the same coordinate reference
system (CRS).
...
Since the two objects share the same CRS we don't need to do anything. 

The 'ID' column in the `circles_sf` object  is `city`, and here we add an empty
column of that name to the `asia_sf` object so that we can combine the two
objects.
...
```{r}
comb_df <- create_map_table(as(comb_sf, 'Spatial'), 
                            IDcolumn = "city")
...
### Micromap showing basemap {#Ch6-Example1Micromap2}

Now we can create the micromap with the basemap displayed. To properly
...
which provides the name the county seat, as well as its elevation, population,
...
Next we want to load the spatial data set showing the polygons of counties
...
in the micromap (i.e., so there is limited overlap), versus showing the correct positions of 
the points. In the example we provided in section \@ref(#Ch6-Example1)
this wasn't an issue because the points of interest were well spaced 
apart. 

Here there are several county seats we could consider moving apart.
...
Now that we have adjusted the point locations, we need to create circular
polgyons around the points, that will be displayed on the micromap. 


```{r fig.cap="Circles drawn around the locations of county seats. No that overlap between individual circles is limited."}
seats_circles <- points2circles(seats_adjust1, radius = 8)
plot(seats_circles$geometry)
```

Next the `seats_circles` object needs to be combined
...
column (i.e., the coordinates), and convert the object to a data frame. 

Refer back to \@ref(#Ch6-Example1Micromap2) and Chapters X and Y for more details
on arguments used in the `mmplot` function.
...
Here we show a micrmap with the actual stadium locations plots 
and use a more automated approach to repel locations of stadiums that are too close together so that
they are visible in the micromap. 
...
We can extract an basemap of the UK, and crop it to the extent we are 
interested. 
      ...
      panel.types = c("labels", "dot", "map", "labels"),
       ...
       panel.att = list(
         ...
         list(3,header = "Light Gray Means\nHighlighted Above",
              ...
              nodata.border.size = 0.5,
              outer.hull.size = 0.5),
         list(4, 'Team', panel.width = 1.8, align = 'left'))
)
```
