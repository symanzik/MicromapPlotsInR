...
for each location. Additionally, as with other micromaps, there is a space constraint and in 
general we find it challenging to display more than roughly
50 point locations in one micromap. 

It is also important to consider whether a micromap of point locations is actually 
...
not points, on maps. Thereby, each point becomes a small polygon 
...
Therefore, the object created in the previous step (which may, for example, be an object from  **sf** package)
needs to be flattened into a dataframe. 

1. Create an initial simple micromap. To make trouble shooting easier, we recommend to that at first you not add all visualization elements (e.g., error bars, labels, multiple variables). 
...
simple features (`sf`) object (from the **sf** package).
...
and we can specify that using the `crs` argument. Correctly specifying the crs is especially 
important for section \@ref(Ch6-Example2Combine) where we combine this object
with a basemap. 

In this step we also use the `sf::st_transform` function to transform the object
in to a projection that is more suitable for mapping locations in Asia. 
This transformation has the added benefit of making `st_crop()` work smoothly in a later cropping step (the data are now projected onto a plane unlike the original unprojected latitude and longitude coordinates).
...
are special kinds of dataframes, that are able to represent a wide variety
...
using `create_map_table()` (see section \@ref(Ch2-Steps)). To achieve this the`sf` object first needs to be converted to a `Spatial` object (from the `sp` package),
...
This dataset can be downloaded using the `rnaturalearth` package. 
...
We then crop the the map based on the extent of the `circles_sf` object we previously created.
Note that we are using the `st_bbox` to get the bounding box of the `circles_sf` object, and then cropping to the extent
...
to the `asia` object. When creating micromaps the input includes an 'ID' column
that contains names that uniquely identify the polygons for which data will be shown (see Chapter X [*Juergen*--do you know what section we should reference here?]).
...
Next we need convert the object into a regular dataframe using `create_map_table` so that it can be used
...
The `mmplot` ducumentation (`?mmplot`), provides information on additional arguments.
...
*Juergen* thoughts on what to include in this section? 
