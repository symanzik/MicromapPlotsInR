...
stadium_circles <- points2circles(stadiums)
```
...
```{r}
stadium_circles$ID <- stadiums$stadium
stadiums_map1 <- rbind(uk_map_sf['ID'], stadium_circles['ID'])
...
don't all over-plot each other. Default arguments in `point_repel()` can be changed
to adjust the level of repulsion between points. 

```{r}
stadiums_repelled <- point_repel(stadiums)
par(mfrow = c(1, 2))
plot(stadiums$geometry, main = 'original point locations')
plot(stadiums_repelled$geometry, main = 'repelled points')
```
