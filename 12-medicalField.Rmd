...
One of the successes in cancer screening has been the continuing decline of colorectal cancer (CRC) rates in the U.S. since screening methods became available about 50 years ago [@SFA2017]\index{Colorectal cancer!Time trends}. A colonoscopy can find and remove pre-cancerous polyps before they become malignant, thus reducing both incidence and mortality of CRC. However, it has become apparent recently that rates among people under age 50, the recommended age to start screening, have started to rise not just in the U.S. but around the world [@CCBlog2020]\index{Colorectal cancer!Early onset}. As a result, many countries have recently lowered the initial screening age to 45. Researchers are puzzled by the increase in rates in the younger group [@CCBlog2025]. Risk factors identified for CRC occurrence in older adults include obesity, a sedentary lifestyle, smoking, excessive alcohol use and first degree relatives who also had CRC\index{Colorectal cancer!Risk factors}. Aspirin use, hormonal therapy and pre-cancerous polyp removal during screening are thought to reduce the risk of CRC. [@CRCPDQ2025]. In this chapter, we will explore CRC mortality data using the **micromapST**\index{R Packages!micromapST} R package [@CP2015CRAN], accessible at https://cran.r-project.org/web/packages/micromapST/index.html to see if the plots can shed light on this new trend.

### Data {#Ch12-Data} 
Mortality data for U.S. states were obtained from the Centers for Disease Control and Prevention (CDC) \index{Centers for Disease Control and Prevention} online Wonder system [@CDCWonder2025]; rates are directly age-adjusted using the U.S. 2000 standard population. The rate in any strata, e.g., by state, age (<50, 50+), sex, race or year, was suppressed (shown as NA) in the original files if the number of cases was less than 16.\index{Data suppression} Mortality data by state were available for 1999-2021, which were grouped into periods 1999-2001, 2002-2006, 2007-2011, 2012-2016, 2017-2021 for trend analysis. This aggregation over time eliminated most of the suppressed state counts. However, the District of Columbia (DC) still had suppressed counts and so was eliminated from the data file. Annual U.S. mortality rates for 1990-2022 were obtained from the NCI State Cancer Profiles web site [@StCaPro2025].\index{National Cancer Institute!State Cancer Profiles} All rates are shown as the number of deaths per 100,000 population.

Not all of the risk and protective factors of interest were available from a nationwide survey that was reasonably representative of each state's population. However, the CDC's Behavioral Risk Factor Surveillance System (BRFSS) [@StCaPro2025] provided state-level survey data for:
...
- Poverty: persons living below the federal poverty level in each state. [@ACSPov2025;@StCaPro2025]
...
As we saw in Chapter \@ref(Ch4b), the **micromapST**\index{R Packages!micromapST} package is no longer limited to displaying U.S. states. To illustrate, we will explore patterns in colorectal cancer mortality rates among those under age 50 in Kentucky. Low rates in this age group preclude examination by county, so we will display rates by Area Development District (ADD), aggregations of the 120 Kentucky counties [@KCADD2024]\index{Kentucky!Area Development Districts}. First, we need to create a working file of mortality and risk factor data. The border group (boundary file) used for these displays of Kentucky ADD data was created in Chapter \@ref(Ch4b).

```{r Ch12-KYADDAllPrint,eval=FALSE}
#
#  To see the name, abbr and ID values that can be used in the data:
#
library(micromapST)
library(stringr)
#jDir     <- "h:/juergenMicromapBook/RProjectFile/MicromapPlotsInR-master/"
jDir <- ""

#dataDir  <- "h:/juergenMicromapBook/HealthChapter/"
dataDir <- ""

dataFile <- "data/Ch12-data/KYCRCData.csv"

outDir     <- "output/Ch12/"
oDir     <- paste0(jDir,outDir)

bdDir    <- "data/Ch4b/"
bdGroup  <- "KYADDBG"
BordDir  <- paste0(jDir,bdDir)
...
#
# Risk Data
RiskName  <- "KYADDRiskFactors.csv"
MortName  <- "CRC Mortality Rates by KY ADDs.csv"

KYADDMort <- read.csv(paste0(dataDir,MortName),header=TRUE,stringsAsFactors=FALSE)
KYADDRisk <- read.csv(paste0(dataDir,RiskName),header=TRUE,stringsAsFactors=FALSE)
...
```{r Ch12-KYLMMRates, fig.cap='Rates and time trends of colorectal cancer mortality among whites ages <50 by Area Development Districts in Kentucky', fig.width=8, fig.height=10,eval=FALSE}

# NOTE: NA data is present in the 4yr and 5yr groups.  Buffalo Trace, FIVCO, Gateway, Kentucky River
#    Pennyrile and Purchase.

#  Assumptions are the rows will have the ADD abbreviation as the row.name and 
#  the columns provided are: RateYr5, and RateYr4 for the rates and KYCRCScreen, KYobese, 
#  KYpoverty, and KYcursmk as the percentage values.
#
...
panelHC11 <- data.frame(type=c("mapcum", "id", "dot",          "arrow"),
 ...
 micromapST(KYADDAllData,panelHC11,sortVar="RateYr5",ascend=FALSE, 
       ...
       bordDir=BordDir, bordGrp=bdGroup)
```       

Even after aggregating the data to 15 ADDs, there are four ADDs with supressed mortality rates, i.e., with fewer than 10 deaths\index{Data suppression}. These ADDS are in eastern Kentucky, an area found to have many poor health outcomes and poor health behaviors in a recent assessment by the Kentucky Department for Public Health [@KYHealth2023]. These four ADDs also have the smallest populations among the 15 ADDs and so it is not surprising that only a few deaths occurred in each area due to a rare cause. 
...
```{r Ch12-KYLMMRisk, fig.cap='Rates and risk factors of colorectal cancer mortality among whites ages <50 by Area Development Districts in Kentucky', fig.width=8, fig.height=10,eval=FALSE}
...
An aggregation of Kentucky counties into an Appalachian versus non-Appalachian region is often used for studies in that area [@AppRegion]. 

```{r Ch12-KYAppMap, echo=FALSE, fig.cap='Appalachian counties in Kentucky', out.width="75%"}
knitr::include_graphics(path = "img/KYAppCounties.pdf")
```

Previous research has shown significant differences between all-age colorectal cancer rates in Appalachian versus non-Appalachian counties in Kentucky [@KYAppRates], with Appalachian incidence and mortality rates being 20% and 13% higher, respectively [@KYAppRates]. Thus, while our linked micromap plots were hampered by suppressed data due to small numbers of deaths in eastern Kentucky ADDs, results of the visual exploratory analysis are consistent with analyses at a broader geographic level. In addition, residents of Appalachian counties were more likely to be obese and to smoke and were less likely to have been screened for CRC [@KYAppRates].
...
Readers interested in delving further into health applications such as those described in this chapter are referred to an epidemiology textbook, such as the classic Gordis Epidemiology [@GordisEpi] or the more practical Essential Epidemiology in Public Health. [@AschEpi]
