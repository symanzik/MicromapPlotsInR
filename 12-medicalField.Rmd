...
MortData  <- read.csv(paste0(dDir,File1), header=TRUE, stringsAsFactors = FALSE)
RiskData  <- read.csv(paste0(dDir,File2), header=TRUE, stringsAsFactors = FALSE)
MAbbr     <- MortData[,2]
RAbbr     <- RiskData[,1]
Mord      <- order(MAbbr)
Rord      <- order(RAbbr)
...
pdf(paste0(dDir,"HC02-TimeTrendsofColorectalCancerMortalityAmongWhitesAg
eslessthan50SPctChgRate.pdf"),height=10.5,width=7.5)
...
```
{r Ch12-StateLMMRisks, fig.cap = 'Colorectal Cancer Mortality Among Whites, < 50, and related 
lifestyle factors', fig.width = 7, fig.height = 4}
...
```
{r Ch12-StateLMMAll, fig.cap = 'Colorectal Cancer Mortality among Whites, ages < 50","Pct Poverty, Pct Obese, Pct No Exer', fig.width = 7, fig.height = 4}
