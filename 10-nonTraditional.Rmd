...
The shapefiles for the microposter plot is the boundaries of the areas of interests (AOIs) for the poster, such as the title, logos, multiple text areas, images, and tables. 
...
if (sortby == "pupildat") {
  ...
  posterdat <- posterdat[order(posterdat[, sortby], 
                               decreasing = decreasing), ]
}
```
...
panels <- panelLayout(
  nrow = rows, ncol = columns, topMargin = details$top, bottomMargin = details$bot, leftMargin = details$left, rightMargin = details$right, rowSep = rep(0.01, rows + 1), rowSize = rowsize,
...
microposters <- function(panel.col,
  ...
  # 7.1 Assign polygon ids (one per polygon)
  ...
  for (i in 1:nGroups) {
    if (MedianRow && i == ceiling(nGroups / 2)) {
...
if (sum(panel.types == "poster") >= 1) {
  ...
  for (i in 1:length(posters)) {
    microposters(
      panel.col = columns.att[posters[i]]$poster$col.num,
...
Now the `m.plot()` function can be executed and the labels and the legends columns can be plotted.
...
if (sum(panel.types == "legend") >= 1) {
  ...
  for (i in 1:length(legends)) {
    m.labels(
      panel.col = columns.att[legends[i]]$legend$col.num,
  ...
  for (i in 1:nGroups) {
    ...
    if (i == nGroups) {
      if (is.na(axis.labels)) {
        graphics::axis(
          side = 1, at = countGrid, labels = as.character(countGrid), col = "black", mgp = c(1, 0, 0),
          tck = -0.04, cex.axis = cex
        )
      } else {
        graphics::axis(
          side = 1, at = countGrid, labels = as.character(axis.labels), col = "black", mgp = c(1, 0, 0),
          tck = -0.04, cex.axis = cex
        )
      }
    }
...
if (sum(panel.types == "dot") >= 1) {
  ...
  for (i in 1:length(dots)) {
    m.dotplot(
      var = columns.att[dots[i]]$dot$panel.data,
      posterdat,
...
Then the final column, the boxplot column, is added.
...
if (sum(panel.types == "boxplot") >= 1) {
  ...
  for (i in 1:length(bplot)) {
    m.boxplot(
      dat = columns.att[bplot[i]]$boxplot$panel.data,
