...
panels <- panelLayout(
  ...
  colSize = panel_width,
  rowSize = rowsize,
  colSep = rep(0.1, (columns + 1)), # space between panels in columns
  rowSep = rep(0.01, rows + 1) # space between panels in rows
)
...
plot_microposters <- function(panel_col,
  ...
  for (i in 1:n_groups) {
    ...
    graphics::polygon(
      ...
      col = scales::alpha(colour = color_scheme[pen], alpha = 0.7),
      border = FALSE
    )
    ...
    if (cumulate) {
      ...
      graphics::polygon(
        ...
        col = scales::alpha(colour = details$wyellow, alpha = 0.7),
        border = FALSE
      )
      graphics::polygon(
        x = aoi_vis_borders$x[past],
...
plot_labels <- function(panel_col,
  ...
  # 8.2 Plot median row panel if there is a median row
  if (median_row) {
    graphics::mtext(
      text = title,
      side = 3,
  ...
  for (i in 1:n_groups) {
    ...
    # 8.3.2 Define the index for the median row if there is a median row
...
main.title <- "Eye Tracking: Statistics from Looking at a Scientific Poster "
title.cex <- 1.08
