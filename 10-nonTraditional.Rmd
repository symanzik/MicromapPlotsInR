...
The shapefile for the microposter plot is the boundaries of the areas of interests (AOIs) for the poster, such as the title, logos, multiple text areas, images, and tables. 
...
After all the settings are specified, we can now start to create the linked microposter plot one column at a time, by starting from the leftmost column, i.e., the microposters. The microposter plots code is structured similar to the `mmplot()` function provided in the **micromap**\index{R Packages!micromap} R package by @PaOl2015.     
...
Now we can apply the `plot_microposters()` function to create our first column in the linked microposter plot, i.e., the microposter column. The color palette is selected from the **RColorBrewer**\index{R Packages!RColorBrewer} R package. In this example, five colors are used from the `Accent` color palette, since we have five rows at most for each perceptual group. Multiple microposter columns could be created if needed.

```{r Ch10-create-microposters, fig.cap = 'Linked microposters plot poster column.'}
color_scheme <- rev(RColorBrewer::brewer.pal(5, "Accent"))
...
plot_labels <- function(panel_col = panel_col,
                     title = title,
                     header_size = 0.7,
                     cex = 0.7,
                     dcex = 0.96,
...
```{r Ch10-create-microposters-and-labels, echo = FALSE, fig.cap = 'Similar to Figure \\@ref(fig:Ch10-create-microposters), now with labels added.'}
...
plot_dotplot <- function(var = var,
  ...
  # B.9.2 Set up panels as big frame for the entire plot 
      ...
      if (outlier[m]) {
        ...
        if (gnams[k] == "Blank") {
          graphics::points(vals, rep(ht, length(vals)),
            ...
            cex = cex_outlier, lwd = lwd_outlier
          )
        } else {
          graphics::points(vals, rep(ht, length(vals)),
            pch = 1,
...
```{r Ch10-dotplot-cum, echo = FALSE, fig.cap = 'Similar to Figure \\@ref(fig:Ch10-create-microposters-and-labels), now with two dotplot columns added.'}
...
```{r Ch10-dotplot-boxplot-cum, echo = FALSE, fig.cap = 'Similar to Figure \\@ref(fig:Ch10-dotplot-cum), now with one boxplot column added.'}
<<Ch10-dotplot-cum>>
<<Ch10-add-boxplot>>
```
...
By now, all the columns in the linked microposter plot have been created. The plot can now be finalized by adding the main title.  
...
Further, all the steps demonstrated in this chapter have been put into function `DrawEyeLMPlot()` in the **EyeTrackR**\index{R Packages!EyeTrackR} that is available on Github (https://github.com/ChunyangCLi/EyeTrackR.git). If the user wants to put overlays on the map differently without the polygons, `DrawEyeLTMPlot()` function provides a scanpath microposter plot without predefined AOIs those code he user can refer to, while the non-traditional micromap creation approach is the same as demonstrated in this book chapter. 
