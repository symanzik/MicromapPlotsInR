  ...
  CreatePStat <- function(obj, nms, j, xlim, ylim,
    ...
    if (pstat.function == "mean") {
      ...
      list("sp.text",
        # c(-83, 34.5),
        textloc,
        # sprintf("mean = %.1f", mean(obj[[nms[j]]], na.rm = TRUE)),
        # bquote(bar(x) == .(pmean)),
        # expression(bar(x) == pmean),
        # TeX(sprintf(r'($\overline{x} = %f$)', pmean)),
        # TeX(r'($\overline{x} = $)'),
        # bquote("Boxplot for MFE, h" <= ~ .(pmean)),
        as.expression(bquote(bar(x) == .(pmean))),
        # paste("avg = ", round(mean(obj[[nms[j]]], na.rm = TRUE), pstat.digits)),
        which = j
      )
      # check here:
      ...
      # https://kevcaz.insileco.io/notes/2019/08/2019-08-02_variables_in_mathematical/
      # https://stackoverflow.com/questions/24130448/bquote-how-to-include-an-expression-saved-as-a-string-object
    } else if (pstat.function == "median") {
      list("sp.text",
        # c(-83, 34.5),
        textloc,
        # sprintf("median = %.1f", median(obj[[nms[j]]], na.rm = TRUE)),
  ...
  if (ncc == 1) {
    ...
    if (pstat.function != "none") {
      ...
      for (j in 1:nlcc[1]) {
        ...
        #       sp.layout[[j]] <- list("sp.text",
        #                                c(-83, 34.5),
        #                                sprintf("%.1f", mean(obj[[nms[j]]], na.rm = TRUE)), which = j)
      }
    }
    ...
    if (fcc[1]) {
      ...
      lattice:::print.trellis(
        x = spplot(obj,
          ...
          aspect = aspect,
          sp.layout = sp.layout,
          # sp.layout = list(list("sp.text", c(-83, 34.5),
          #                       sprintf("%.1f", mean(obj[[nms[1]]], na.rm = TRUE)), which = 1),
          #                  list("sp.text", c(-84, 34.5), "B", which = 2),
          #                  list("sp.text", c(-84, 34.5), "C", which = 3),
          #                  list("sp.text", c(-83, 34.5),
          #                       sprintf("%.1f", mean(obj[[nms[4]]], na.rm = TRUE)), which = 4)),
          xlim = xlim, ylim = ylim,
          strip = strip.custom(
            which.given = 1,
            shingle.intervals = as.matrix(lcc[[1]]),
