...
myCCmaps <- function(
    ...
    aspect = mapasp(obj, xlim, ylim), sp.layout = NULL, xlim = bbox(obj)[1, ], ylim = bbox(obj)[2, ]) {
  ...
  if (ncc == 1) {
    if (fcc[1]) {
      ...
      for (j in 1:nlcc[1]) {
        ...
        io <- Outside(cvar[[1]], ilcc[j, 1], ilcc[
          j,
          2
        ])
        obj[[vn]] <- obj[[zcol]]
      ...
      if (fcc[1]) {
        ...
        for (i in 1:nlcc[1]) {
          for (j in 1:nlcc[2]) {
            ...
            obj[[vn]] <- obj[[zcol]]
            ioi <- as.character(cvar[[1]]) != lcc[[1]][i]
            ioj <- Outside(cvar[[2]], jlcc[j, 1], jlcc[
              j,
              2
            ])
            io <- ioi | ioj
        ...
        for (i in 1:nlcc[1]) {
          for (j in 1:nlcc[2]) {
            ...
            obj[[vn]] <- obj[[zcol]]
            ioi <- Outside(cvar[[1]], ilcc[i, 1], ilcc[
              i,
              2
            ])
            ioj <- as.character(cvar[[2]]) != lcc[[2]][j]
